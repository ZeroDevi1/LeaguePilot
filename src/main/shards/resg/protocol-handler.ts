...
/** RESG 版本索引路径。 */
const VERSION_INDEX_PATH = '/api/v1/versions.json'
/** RESG 英雄列表路径。 */
const CHAMPION_INDEX_PATH = /^\/api\/v1\/versions\/\d+(?:\.\d+)*\/champions\.json$/
/** RESG 单英雄详情路径；禁止代理任意上游 URL。 */
const CHAMPION_DETAIL_PATH = /^\/api\/v1\/versions\/\d+(?:\.\d+)*\/champions\/[1-9]\d*\.json$/
 ...
 * @returns 上游响应，或本地生成的 404、405、502 响应。
 */
